...
def test_zh_character():
    out = ZhProcessor(headers).zh_character("你")
...
def test_zh_characters():
    out = ZhProcessor(headers).zh_character("你好")
    # after quote_code there is a random hash, so get rid of it
    modified_out = re.sub(r"quote_code=.*?\)", "quote_code=)", out)
    print(repr(modified_out))
    assert modified_out in [
        '# 你好\n\nCharacter  | [你](https://en.wiktionary.org/wiki/你#Chinese) | [好](https://en.wiktionary.org/wiki/好#Chinese)\n---|---|---|\n**Mandarin** | ** | **\n**Cantonese**  | ** | **\n**Meanings**  | "." | "."\n\n\n^Information ^from [^(Unihan)](https://www.unicode.org/cgi-bin/GetUnihanData.pl?codepoint=你好) ^| [^(CantoDict)](https://www.cantonese.sheik.co.uk/dictionary/characters/你好/) ^| [^(Chinese Etymology)](https://hanziyuan.net/#你好) ^| [^(CHISE)](https://www.chise.org/est/view/char/你好) ^| [^(CTEXT)](https://ctext.org/dictionary.pl?if=en&char=你好) ^| [^(MDBG)](https://www.mdbg.net/chinese/dictionary?page=worddict&wdrst=1&wdqb=你好) ^| [^(MoE DICT)](https://www.moedict.tw/\'你好) ^| [^(MFCCD)](https://humanum.arts.cuhk.edu.hk/Lexis/lexi-mf/search.php?word=你好)'
    ]


def test_zh_word1():
    assert (
        ZhProcessor(headers).zh_word("螃蟹")
...
def test_zh_word2():
    assert (
        ZhProcessor(headers).zh_word("佛陀")
...
def test_zh_word3():
    assert (
        ZhProcessor(headers).zh_word("为所欲为")
...
def test_zh_word4():
    assert (
        ZhProcessor(headers).zh_word("公道杯")
        == '# [公道杯](https://en.wiktionary.org/wiki/公道杯#Chinese)\n\nLanguage | Pronunciation\n---------|--------------\n**Mandarin** (Pinyin) | *gōngdàobēi*\n**Mandarin** (Wade-Giles) | *kung^(1) tao^(4) pei^(1)*\n**Mandarin** (Yale) | *gung^(1) dau^(4) bei^(1)*\n**Cantonese** | *---*\n\n\n**Tea Meanings**: " Justice Pot, a vessel used in gongfu tea preparation into which the liquor is quickly decanted from the brewing pot so it can be distributed fairly to everyone drinking the liquor, all receiving the same strength beverage." ([Babelcarp](http://babelcarp.org/babelcarp/babelcarp.cgi?phrase=公道杯&define=1))\n\n\n\n\n^Information ^from [^CantoDict](https://www.cantonese.sheik.co.uk/dictionary/search/?searchtype=1&text=公道杯) ^| [^MDBG](https://www.mdbg.net/chinese/dictionary?page=worddict&wdrst=0&wdqb=c:公道杯) ^| [^Yellowbridge](https://yellowbridge.com/chinese/dictionary.php?word=公道杯) ^| [^Youdao](https://dict.youdao.com/w/eng/公道杯/#keyfrom=dict2.index)'
    )
