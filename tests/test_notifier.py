...
def test_ziwen_message_subscribe(mock_config):
    ...
    mock_config.reddit.inbox.unread.return_value = generate_mock_message("SUBSCRIBE")
...
@patch("code.notifier.action_counter")
def test_ziwen_message_ping(mock_config):
    mock_config.is_mod.return_value = False
    mock_config.reddit.inbox = MagicMock(praw.reddit.models.Inbox)
    mock_messages = generate_mock_message("PING")
    mock_config.reddit.inbox.unread.return_value = mock_messages
    mock_config.cursor_main.fetchone.return_value = {"user_count": 0}
    ziwen_messages(mock_config)
    mock_messages[0].reply.assert_called_with(
...
def test_ziwen_message_unsubscribe(mock_config):
    mock_config.reddit.inbox = MagicMock(praw.reddit.models.Inbox)
    mock_config.reddit.inbox.unread.return_value = generate_mock_message("UNSUBSCRIBE")
...
@patch("code.notifier.action_counter")
def test_ziwen_message_status(mock_config):
    mock_config.reddit.inbox = MagicMock(praw.reddit.models.Inbox)
    mock_messages = generate_mock_message("STATUS")
    mock_config.reddit.inbox.unread.return_value = mock_messages
